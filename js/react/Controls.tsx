...
import { init, setSimCount, setStarCount, setPause, setOptimization, setDT, setTheta } from "../threejs/particles";
...
export default function Controls(props: Props) {
  ...
  then remove notification accordingly */
	...
	return (
		<>
			<ShimmerText id={id} className={["upper notification", isShown ? "out" : "shown"].join(" ")}>
				Click to create black hole
			</ShimmerText>
			<form className={["controls-panel upper", Optimization[opt], isShown ? "shown" : "out"].join(" ")}>
				<Button
					onClick={() => {
						setShown(!isShown);
					}}
				...
				<section className="text-content">
					...
					<Button className="reset faded" onClick={() => setSimCount(simulatedCount)}>
						Reset
					</Button>
						...
						onClick={(o) => {
							...
							switch (o) {
								case Optimization.BarnesHut:
									sSimC(clamp(simulatedCount, 0, MAX_NUM_BARNES_SIMULATE));
									break;
								case Optimization.Direct:
									sSimC(clamp(simulatedCount, 0, MAX_NUM_DIRECT_SIMULATE));
									break;
							}
						}}
					>
						<Button groupLabel={Optimization.BarnesHut}>Barnes Hut</Button>
						<Button groupLabel={Optimization.Direct}>Direct</Button>
					</ButtonGroup>
					<Slider
						min={0}
						max={opt == Optimization.BarnesHut ? MAX_NUM_BARNES_SIMULATE : MAX_NUM_DIRECT_SIMULATE}
						value={simulatedCount}
						onChange={sSimC}
						label="Particles"
					/>
					<Slider min={MIN_THETA} max={MAX_THETA} step={0.1} value={theta} onChange={sT} className="theta" label="Threshold" />
					<Slider min={MIN_DT} max={MAX_DT} step={0.1} value={dt} onChange={sDT} label="Delta Time" />
					<Slider min={0} max={MAX_NUM_STARS} value={starCount} onChange={sStarC} label="Stars" />
				</section>
			</form>
		</>
	);
